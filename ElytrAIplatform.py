...
class elytraFlyer(gym.Env):
    ...
    def __init__(self, env_config, log_frequency=1, move_mult=50, ):
        ...
        self.observation_space = Box(-10000, 10000, shape=(12,), dtype=np.float32)
    ...
    def step(self, action):
        ...
        ideal_landing_x = self.platx + 12.5
        ideal_landing_y = self.platy
        ideal_landing_z = self.platz + 12.5

        # if the agent isnt moving and it isnt at it's start height
        if self.obs[3] == 0 and self.obs[4] == 0 and self.obs[5] == 0 and self.obs[1] < 61:
            # if agent position hasnt been reset to 000
            if self.obs[0] != 0 and self.obs[1] != 0 and self.obs[2] != 0:
                xPercent = self.obs[0] / ideal_landing_x
                yPercent = self.obs[1] / ideal_landing_y
                zPercent = self.obs[2] / ideal_landing_z

                # if negative position
                if self.obs[0] < 0:
                    xPos = (-1 * self.obs[0]) + ideal_landing_x
                    xPercent = (-1 * xPos) / ideal_landing_x
                if self.obs[1] < 0:
                    yPos = (-1 * self.obs[1]) + ideal_landing_y
                    yPercent = (-1 *yPos) / ideal_landing_y
                if self.obs[2] < 0:
                    zPos = (-1 * self.obs[2]) + ideal_landing_z
                    zPercent = (-1 * zPos) / ideal_landing_z

                # if flying past the platform
                if self.obs[0] > ideal_landing_x:
                    xPos = self.obs[0] - ideal_landing_x
                    xPercent = xPos / ideal_landing_x
                if self.obs[1] > ideal_landing_y:
                    yPos = self.obs[1] - ideal_landing_y
                    yPercent = yPos / ideal_landing_y
                if self.obs[2] > ideal_landing_z:
                    zPos = self.obs[2] - ideal_landing_z
                    zPercent = zPos / ideal_landing_z

                aiPercent = (xPercent + yPercent + zPercent) / 3
                self.saved_plat_reward = aiPercent

                print("agent pos: ", self.obs[0], " ", self.obs[1], " ", self.obs[2])
                print("score: ", aiPercent)
        ...
        while world_state.is_mission_running:
            ...
            if world_state.number_of_observations_since_last_state > 0:
                ...
                obsList = [xPos, yPos, zPos, xVelocity, yVelocity, zVelocity, blockInSightDistance, blockInSightX,
                           blockInSightY, blockInSightZ, pitch, yaw]
