...
class elytraFlyer(gym.Env):
    ...
    def __init__(self, env_config):
        ...
        self.observation_space = Box(0, 255, shape=(input_layers, self.video_width, self.video_height),
                                     dtype=np.float32)
        ...
        self.walls_touched_in_run = 0
        self.num_pillars_passed = 0
        ...
        if env_config == {}:
            ...
            self.walls_touched = env_config["walls_touched"]
            self.pillars_passed = env_config["pillars_passed"]
    ...
    def append_episode_scores(self):
        ...
        self.walls_touched.append(self.walls_touched_in_run)
        self.pillars_passed.append(self.num_pillars_passed)
    ...
    def calculate_reward(self, world_state):
        ...
        for r in world_state.rewards:
            ...
            if r.getValue() == self.wall_touch_flag or self.current_x >= 28 or self.current_x <= -28:
                print("xml wall touch register")
                reward = reward + self.wall_hit_reward_add
                self.walls_touched_in_run += 1

        if self.current_z > self.pillar_z[0]:
            passed = self.pillar_z.pop(0)
            print("passed pillar ", passed)
            self.num_pillars_passed += 1
            reward = reward + (self.num_pillars_passed * self.pillar_passed_multiplier)
    ...
    def step(self, action):
        ...
        if self.current_x >= 28 or self.current_x <= -28:
            print("wall touched, teleporting agent")
            self.agent_host.sendCommand(f"tp 0.5 {self.current_y} {self.current_z}")
            reward = reward + self.wall_hit_reward_add
            self.walls_touched_in_run += 1

        if self.current_z < 0.5:
            print("going backwards, die")
            self.agent_host.sendCommand(f"tp 0.5 1 0.5")
            reward = -reward * self.pillar_hit_reward_multiplier
    ...
    def get_pillar_locations(self, width=450, length=450):
        ...
        for z in range(int(length)):
            if abs(z) > 30:
                    # if the z is not 0 and every 20th block, put a pillar
                    if z != 0 and z % 20 == 0:
                        rand_x = randrange(-21, 21)
                        self.pillar_z.append(z)
                        #print("random pillar x ", rand_x)
                        #print("pillar placed at z: ", z)
                        return_string += f"<DrawLine x1='{rand_x}' y1='2' z1='{z}' x2 = '{rand_x}' y2 = '100' z2 = '{z}' type='coal_block'/>\n"
                        rand_x = randrange(-21, 21)
                        return_string += f"<DrawLine x1='{rand_x}' y1='2' z1='{z}' x2 = '{rand_x}' y2 = '100' z2 = '{z}' type='coal_block'/>\n"
        return return_string

    def draw_walls(self, width=31, length=450):
        return_string = ""
        x = width
        for z in range(int(length)):
            #positive wall
        ...
        for x in range(-32, 32):
            #back wall
            return_string += f"<DrawLine x1='{x}' y1='2' z1='-1' x2 = '{x}' y2 = '100' z2 = '-1' type='coal_block'/>\n"
        return return_string
    ...
    def get_mission_XML(self):
        ...
        return '''<?xml version="1.0" encoding="UTF-8" standalone="no" ?>
               ...
               f'<Block reward="{self.wall_touch_flag}" type="stone"/>' + ''' 
                        </RewardForTouchingBlockType>     
                    </AgentHandlers>
                </AgentSection>
                </Mission>
    ...
    def log_returns(self):
        # Log walls hit
        try:
            with open('outputs/WallsTouched.txt', 'w') as f:
                for step, value in zip(self.episodes[1:], self.walls_touched[1:]):
                    f.write("{}\t{}\n".format(step, value))
        except Exception as e:
            print("Unable to log walls touched results in text")
            print(e)

        # Log pillars passed
        try:
            with open('outputs/PillarsPassed.txt', 'w') as f:
                for step, value in zip(self.episodes[1:], self.pillars_passed[1:]):
                    f.write("{}\t{}\n".format(step, value))
        except Exception as e:
            print("Unable to log pillars passed results in text")
            print(e)
